...
from src.tokenizer.models.utils_diffusion import timestep_embedding
from src.tokenizer.common import checkpoint
from src.tokenizer.basics import (
    zero_module,
    conv_nd,
    linear,
    avg_pool_nd,
...
from src.tokenizer.modules.attention import SpatialTransformer, TemporalTransformer
...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, context=None, features_adapter=None, fs=None, **kwargs):
        ...
        # !!! We always use per-frame image conditioning
        # Starting from 28-Oct-2024
        num_tokens = (l_context - 77) // t
        assert (l_context - 77) % num_tokens == 0, \
            'Wrong context shape, x.shape: {}, context.shape: {}'.format(x.shape, context.shape)
        ...
        #     assert False, ("Perframe", x.shape, context.shape)
        #     context_text, context_img = context[:,:77,:], context[:,77:,:]
        #     context_text = context_text.repeat_interleave(repeats=t, dim=0)
        #     context_img = rearrange(context_img, 'b (t l) c -> (b t) l c', t=t)
        #     context = torch.cat([context_text, context_img], dim=1)
        # else:
        #     assert False, ("Repeat", x.shape, context.shape)
