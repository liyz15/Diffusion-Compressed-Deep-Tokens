...
import torch
from src.tokenizer.models.utils_diffusion import make_ddim_sampling_parameters, make_ddim_timesteps, rescale_noise_cfg
from src.tokenizer.common import noise_like
from src.tokenizer.common import extract_into_tensor
import copy
...
class DDIMSampler(object):
    ...
    @torch.no_grad()
    def fifo_onestep(self, cond, shape, latents=None, timesteps=None, index=None,
                     unconditional_guidance_scale=1., unconditional_conditioning=None,
                     guidance_rescale=0.0, use_original_steps=False, **kwargs):
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
            model_output = self.model.apply_model(latents, ts, cond, **kwargs) # unet denoiser
        else:
            ### do_classifier_free_guidance
            if isinstance(cond, torch.Tensor) or isinstance(cond, dict):
                e_t_cond = self.model.apply_model(latents, ts, cond, **kwargs)
                e_t_uncond = self.model.apply_model(latents, ts, unconditional_conditioning, **kwargs)
            else:
                raise NotImplementedError
        ...
        if self.model.parameterization == "v":
            e_t = self.model.predict_eps_from_z_and_v(latents, ts, model_output)
        else:
            e_t = model_output
        ...
        # sqrt_one_minus_at = torch.full(size, sqrt_one_minus_alphas[index],device=device)
        # assert False, (
        #     alphas.shape, alphas_prev.shape, sigmas.shape, sqrt_one_minus_alphas.shape, size,
        #     alphas[index].shape, alphas_prev[index].shape, sigmas[index].shape, sqrt_one_minus_alphas[index].shape
        # )

        a_t = alphas[index].view(size)
        a_prev = torch.tensor(alphas_prev[index], device=alphas.device).view(size)
        ...
        if self.model.parameterization != "v":
            ...
            pred_x0 = self.model.predict_start_from_z_and_v(latents, ts, model_output)
